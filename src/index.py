...
encoderCountLeft = {"leftEncoder":0}
encoderCountRight = {"rightEncoder":0}
...
----------------------Thread Methods----------------------------
'''    
def readLeftEncoder(s, count):
    logging.debug('Starting Left Encoder Thread')
    #Update the encoder dictionary constantly
    leftEncoder.readRotors(encoderCountLeft)
    # logging.debug(encoderCountLeft)

def readRightEncoder(s, count):
    logging.debug('Starting Right Encoder Thread')
    #Update the encoder dictionary constantly
    rightEncoder.readRotors(encoderCountRight)
...
def move():
    ...
    while (totalTicks < ticksToMove):
        ...
        totalTicks = encoderCountLeft['leftEncoder']
...
def turn(degrees):
    ...
    while (leftDone == False or rightDone == False):
        if(encoderCountLeft['leftEncoder'] >= ticksToTurnLeft):
            motors.motor1.setSpeed(0)
            leftDone = True
        if(encoderCountRight['rightEncoder'] >= ticksToTurnRight):
